...
module.exports = grammar({
  ...
  rules: {
    source_file: ($) =>
      sepEndBy1(optional(';'), field('declaration', $._declaration)),

    _declaration: ($) =>
      field(
        'declaration',
        choice($.type_decl, $.let_declaration, $.preprocessor)
      ),

    _program: ($) => prec(1, choice($.let_in, $._expr)),
    ...
    let_in: ($) => seq($._declaration, ';', field('body', $._program)),
    ...
    Some_call: ($) =>
      prec.right(10, seq(field('some', $.Some), field('argument', $._expr))),
    ...
    michelson_interop: ($) =>
      seq(
        '[%Michelson',
        par(
          seq(field('code', $.michelson_code), ':', field('type', $._type_expr))
        ),
    ...
    record_field: ($) =>
      seq(field('accessor', $.FieldName), ':', field('value', $._program)),

    record_field_path: ($) =>
      seq($._accessor_chain, ':', field('value', $._program)),
    ...
    _accessor_chain: ($) =>
      prec.right(11, sepBy1('.', field('accessor', $.FieldName))),

    list: ($) => brackets(sepBy(',', field('element', $._spread_expr))),

    _spread_expr: ($) => choice($._expr, $.spread),
    ...
    switch: ($) =>
      seq(
        ...
        block(seq(optional('|'), sepBy('|', field('alt', $.alt))))
      ),
    ...
    // a.attribute
    data_projection: ($) => seq(field('expr', $.Name), '.', $._accessor_chain),

    binary_call: ($) =>
      choice(
        ...OPS.map(([op, precendence]) =>
          prec.right(
            precendence,
            seq(
              field('left', $._expr),
    ...
    ...OPS.reduce((acc, [e, _]) => ({ ...acc, [e]: ($) => seq(e) }), {}),

    unary_call: ($) =>
      prec.right(8, seq(field('negate', $.negate), field('arg', $._expr_term))),
    ...
    block: ($) =>
      prec(1, block(seq(sepEndBy(';', field('statement', $._statement))))),

    _statement: ($) => prec(1, choice($.let_declaration, $.type_decl, $._expr)),

    tuple: ($) => par(sepBy1(',', field('item', $._annot_expr))),

    _annot_expr: ($) => choice($.annot_expr, $._expr),

    annot_expr: ($) =>
      seq(field('subject', $._expr), ':', field('type', $._type_expr)),
    ...
    _core_type: ($) =>
      choice($.type_application, $.TypeName, $.Int, $.module_TypeName),
    ...
    record_type: ($) =>
      withAttrs($, block(sepEndBy1(',', field('field', $.field_decl)))),

    field_decl: ($) =>
      prec(
        10, // see 'accessor_chain' for explanation of precedence
        withAttrs(
          $,
          seq(
    ...
    nullary_constr_pattern: ($) => seq(field('constructor', $.ConstrName)),

    unary_constr_pattern: ($) =>
      prec(
        1,
        seq(field('constructor', $.ConstrName), field('arg', $._pattern))
      ),

    constr_pattern: ($) =>
      seq(
        field('ctor', $.ConstrName),
        optional(seq('(', sepBy(',', field('arg', $._pattern)), ')'))
      ),
    ...
    record_field_pattern: ($) =>
      withAttrs(
        $,
        prec(9, seq(field('name', $.FieldName), '=', field('body', $._pattern)))
      ),

    annot_pattern: ($) =>
      seq(field('subject', $._pattern), ':', field('type', $._type_expr)),

    tuple_pattern: ($) =>
      prec(13, par(sepBy1(',', field('pattern', $._pattern)))),

    list_pattern: ($) =>
      brackets(sepBy(',', field('pattern', $._spread_pattern))),

    _spread_pattern: ($) => choice($.spread_pattern, $._pattern),

    spread_pattern: ($) => seq('...', field('expr', $._pattern)),
    ...
    preprocessor: ($) =>
      field(
        ...
        choice($.include, $.p_if, $.p_error, $.p_warning, $.p_define)
      ),
    ...
    p_define: ($) =>
      seq(choice('#define', '#undef'), field('definition', $._till_newline)),
